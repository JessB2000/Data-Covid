...
const folder = process.env.FORMDATA_TOKEN;
...
export class GoFileService{
  ...
  async makeFormData( filepath:string, filename: string){
   ...
   async uploadFormData( fileOne: GoFilePath, fileTwo: GoFilePath ){
    ...
    const oneFormData = await this.makeFormData(fileOne.path, fileOne.name);
    const twoFormData = await this.makeFormData(fileTwo.path, fileTwo.name);
